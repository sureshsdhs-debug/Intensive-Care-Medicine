...
const QuestionWidget = () => {
  ...
  const fetchAllQuestion = async () => {
    try {
      ...
      console.error(err);
      toast.error(err?.response?.data?.message || err.message || "Error fetching questions");
    }
  };


  const fetchThisUserResult = async () => {
    try {
      const { data } = await axios.get(`${BACKEND_BASE_URL}/api/result/get-thisuser-result`,
        { headers: { Authorization: `Bearer ${token}` } }
      );  
      if (Array.isArray(data)) {
        setServerResults(data);
      } else if (data?.success && Array.isArray(data.resultData)) {
        setServerResults(data.resultData);
      } else {
        setServerResults([]);
      }
    } catch (err) {
  ...
  // sync server results into selected/submitted/results state
useEffect(() => {
  if (!questions?.length || !serverResults?.length) return;
  // build maps
  const newSelected = { ...selected }; // keep any existing local selections if you want
  const newSubmitted = new Set(submitted); // keep existing
  const newResults = { ...results };

  serverResults.forEach(r => {
    const qId = r.questionid;
    // find question object for this id
    const qObj = questions.find(q => (q._id ?? q.id) === qId);
    if (!qObj) return; // question not yet loaded (shouldn't happen if we check lengths)

    const selectedKey = r.selectedoption; // e.g. "option3"
    // get option text from question using the key
    const selectedText = qObj[selectedKey] ?? null;

    if (selectedText) {
      newSelected[qId] = selectedText;
      newSubmitted.add(qId);

      // optionally compute correctness if you can resolve correctValue locally
      const correctValue = resolveCorrectValue(qObj); // you already have this helper
      const isCorrect = correctValue != null ? String(selectedText).trim() === String(correctValue).trim() : null;

      newResults[qId] = { isCorrect, correctValue };
    } else {
      // If server stored option key but question doesn't have that key (edge-case), still mark submitted with key
      newSelected[qId] = selectedKey;
      newSubmitted.add(qId);
      newResults[qId] = { isCorrect: null, correctValue: resolveCorrectValue(qObj) ?? null };
    }
  });
  ...
  // answered question ids from server
  ...
  const targetQuestion = nextQuestion || questions[questions.length - 1];

  if (targetQuestion) {
    const targetId = targetQuestion._id ?? targetQuestion.id;
...
// }, [questions, serverResults]);
  ...
  const submitInToDB = async (respData) => {
    try {

      const { data } = await axios.post(`${BACKEND_BASE_URL}/api/result/add`, respData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const resolveCorrectValue = (q) => {
    let correctValue = null;
    if (q && q.correctoption) {
      if (typeof q.correctoption === "string" && q[q.correctoption]) correctValue = q[q.correctoption];
      else {
        const co = Number(q.correctoption);
        if (!Number.isNaN(co)) {
          const opts = [q.option1, q.option2, q.option3, q.option4].filter(Boolean);
          if (co >= 1 && co <= opts.length) correctValue = opts[co - 1];
          else correctValue = opts[co] ?? null;
        }
      }
    }
    if (!correctValue) correctValue = q.correctAnswer ?? q.answer ?? null;
    return correctValue;
  };
  ...
  const submitAnswer = async (q) => {
    ...
    const selectedoption = Object.keys(q).find(key => q[key] === answer);
    ...
    submitInToDB(respData);

    const correctValue = resolveCorrectValue(q);

    const isCorrect = correctValue != null ? String(answer).trim() === String(correctValue).trim() : null;

    setSubmitted(prev => new Set(prev).add(id));
    setResults(prev => ({ ...prev, [id]: { isCorrect, correctValue } }));

    // If correct -> open stats panel automatically
    if (isCorrect === true) {
      setShowStats(prev => ({ ...prev, [id]: true }));
    }

    if (isCorrect === true) toast.success("Correct!");
    else if (isCorrect === false) toast.error("Incorrect.");
    else toast("Submitted — correct answer not available locally.");
                    ...
                    {showStats[id] ? (
                      <div style={{ display: "flex", gap: 24 }}>
                        <div style={{ flex: 1 }}>
                          ...
                          {totalResponses != null && <div style={{ marginTop: 12, color: "#444",fontWeight:600 }}>{totalResponses} Total Responses</div>}
